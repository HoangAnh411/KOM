...
test("season archive requires auth and legacy admin close is disabled or token protected", async () => { const server = createServer(); const originalToken = config.adminToken; try { const seasonId = server.store.snapshot.season.id; config.adminToken = ""; const disabled = await server.app.inject({ method: "POST", url: "/api/admin/season/close", payload: { seasonId, reason: "test close" } }); assert.equal(disabled.statusCode, 404); assert.equal(disabled.json().code, "ADMIN_DISABLED"); config.adminToken = "secret-test-token"; const denied = await server.app.inject({ method: "POST", url: "/api/admin/season/close", headers: { authorization: "Bearer wrong" }, payload: { seasonId, reason: "test close" } }); assert.equal(denied.statusCode, 401); const closed = await server.app.inject({ method: "POST", url: "/api/admin/season/close", headers: { authorization: "Bearer secret-test-token" }, payload: { seasonId, reason: "test close" } }); assert.equal(closed.statusCode, 200); assert.equal(closed.json().status, "finalized"); const privateArchive = await server.app.inject({ method: "GET", url: "/api/season-history" }); assert.equal(privateArchive.statusCode, 401); } finally { config.adminToken = originalToken; await server.app.close(); } });
