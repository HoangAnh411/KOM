...
export class GameStore {
  ...
  async load(options?: { skipLedger?: boolean; strict?: boolean; client?: PoolClient }): Promise<void> {
    const executor = options?.client ?? this.pool;
    let loadedStateUpdatedAt: string | undefined;
    if (executor) {
      try {
        const result = await executor.query<{ state: GameState; updated_at: Date }>("SELECT state, updated_at FROM game_state WHERE state_key = $1", ["kingdom"]);
        if (result.rows[0]?.state) {
          if (result.rows[0].updated_at) loadedStateUpdatedAt = result.rows[0].updated_at.toISOString();
          const { processedCommands: _legacyProcessedCommands, ...saved } = result.rows[0].state as GameState & { processedCommands?: string[] };
          if (saved.season.worldId !== worldAssetId) throw new IncompatibleWorldError(`WORLD_VERSION_MISMATCH: saved world ${saved.season.worldId ?? "meridian-36-v1"} cannot run on ${worldAssetId}. Keep the old save on its matching release; use a separate fresh database for the new world. No save was changed.`);
          this.state = { ...saved, players: saved.players.map(player => ({ ...player, crossSeasonReputation: player.crossSeasonReputation ?? 0 })), cities: saved.cities.map(city => ({ ...city, cityLayoutVersion: (city as any).cityLayoutVersion ?? 1, cityLayoutRevision: (city as any).cityLayoutRevision ?? 0, buildingPlots: city.buildingPlots ?? [] })), armies: (saved.armies ?? []).map(army => ({ ...army, ownerType: army.ownerType ?? "player", ownerPlayerId: army.ownerPlayerId ?? null })), commanders: saved.commanders ?? [], troopReserves: saved.troopReserves ?? {}, formationPresets: saved.formationPresets ?? [], trainingQueues: saved.trainingQueues ?? {}, hospitalQueues: saved.hospitalQueues ?? {}, technologyProgress: saved.technologyProgress ?? {}, researchQueues: saved.researchQueues ?? {}, campaignProgress: saved.campaignProgress ?? {}, activeOperations: saved.activeOperations ?? {}, heroes: saved.heroes ?? [], caravans: [], battleReports: (saved.battleReports ?? []).slice(-battleReportMemoryLimit), terrainMap: saved.terrainMap ?? {}, explorationMasks: saved.explorationMasks ?? {}, militaryThroughput: saved.militaryThroughput ?? {}, regionControl: saved.regionControl ?? {}, regionStates: saved.regionStates ?? {}, rivalIntents: saved.rivalIntents ?? [], regionControlRevision: saved.regionControlRevision ?? 0, alliances: saved.alliances ?? [], allianceVotes: saved.allianceVotes ?? [], treaties: saved.treaties ?? [], diplomacyThroughput: saved.diplomacyThroughput ?? {}, spyMissions: saved.spyMissions ?? [], worldEvents: (saved.worldEvents ?? []).map(event => ({ ...event, seed: event.seed ?? 0, lastPlagueAt: event.eventType === "plague" ? event.lastPlagueAt ?? event.startsAt : event.lastPlagueAt })), counterIntelActive: saved.counterIntelActive ?? {}, seasonMetrics: saved.seasonMetrics ?? { resourcesProduced: {} }, raiderSpawnState: saved.raiderSpawnState ?? { sequence: 0 }, logisticsCounters: saved.logisticsCounters ?? { exports: {}, harvests: {} }, cosmeticAccounts: saved.cosmeticAccounts ?? {}, dailyQuests: saved.dailyQuests ?? {}, activityCounters: saved.activityCounters ?? { trainingBatches: {}, caravansDelivered: {}, campaignsCompleted: {} } };
          for (const city of this.state.cities) normalizeBuildingPlots(city);
          const reps = await executor.query<{ player_id: string; score: number }>("SELECT player_id, score FROM player_reputation WHERE player_id = ANY($1::uuid[])", [this.state.players.map(player => player.id)]);
          for (const row of reps.rows) { const player = this.findPlayer(row.player_id); if (player) player.crossSeasonReputation = row.score; }
        }
      } catch (error) { if (error instanceof IncompatibleWorldError) throw error; if (options?.strict) throw error; console.warn("database load skipped", error instanceof Error ? error.message : error); }
    }
    this.logistics.seed(this.state); this.combat.seed(this.state);
    await this.logistics.load(this.state, executor!, options?.strict);
    await this.combat.load(this.state, executor!, options?.strict);
    await this.diplomacy.load(this.state, executor!, options?.strict);
    await this.espionage.load(this.state, executor!, options?.strict);
    this.worldEvents.seed(this.state); this.raiders.seed(this.state); this.rivals.seed(this.state); ensureRegionStates(this.state);
    await this.onboarding.load(this.state, executor!, options?.strict);
    if (loadedStateUpdatedAt) this.persistedStateUpdatedAt = loadedStateUpdatedAt;
    if (!options?.skipLedger) await this.ledger.load();
  }
  ...
  discardUnpersistedEvents(): void { this.ledger.discardPending(); this.tickReportBatch = []; this.tickCancelBatch = []; }
  async save(): Promise<void> {
    ...
    try {
      ...
      const current = await client.query<{ updated_at: Date }>("SELECT updated_at FROM game_state WHERE state_key='kingdom'");
      const currentUpdatedAt = current.rows[0]?.updated_at.toISOString();
      if (this.persistedStateUpdatedAt && currentUpdatedAt !== this.persistedStateUpdatedAt) throw new Error("STALE_STATE");
      const persistedStateUpdatedAt = await this.persistState(client);
      await client.query("COMMIT");
      this.persistedStateUpdatedAt = persistedStateUpdatedAt;
      this.ledger.markPersisted();
    } catch (error) { await client.query("ROLLBACK").catch(() => undefined); throw error; } finally { client.release(); }
  }
  private async persistState(client: PoolClient): Promise<string | undefined> { await this.logistics.persist(client, this.state); await this.combat.persist(client, this.state); await this.diplomacy.persist(client, this.state); await this.espionage.persist(client, this.state); await this.raiders.persist(client, this.state); await this.onboarding.persist(client, this.state); await this.ledger.persist(client); const saved = await client.query<{ updated_at: Date }>("INSERT INTO game_state (state_key, state) VALUES ($1, $2) ON CONFLICT (state_key) DO UPDATE SET state = EXCLUDED.state, updated_at = now() RETURNING updated_at", ["kingdom", JSON.stringify(this.state)]); return saved.rows[0]?.updated_at.toISOString(); }
  ...
  private async executeCommandUnlocked<T>(event: { eventType: string; aggregateType: string; aggregateId: string; commandId?: string; actorPlayerId: string }, action: () => T): Promise<{ alreadyApplied: boolean; result: T | "already_processed" }> {
    ...
    const client = await this.pool.connect();
    try { await client.query("BEGIN"); await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`state:${this.state.kingdom.id}`]); await this.load({ skipLedger: true, strict: true, client }); previousState = structuredClone(this.state); previousLogistics = this.logistics.capture(); previousEspionage = this.espionage.capture(); previousOnboarding = this.onboarding.capture(); const actor = await client.query("SELECT status FROM players WHERE id=$1", [event.actorPlayerId]); if (actor.rows[0]?.status === "banned") throw new Error("ACCOUNT_BANNED"); if (event.commandId) { const existing = await client.query("SELECT 1 FROM event_ledger WHERE command_id=$1", [event.commandId]); if (existing.rowCount) { await client.query("ROLLBACK"); this.commands.rollback(); return { alreadyApplied: true, result: "already_processed" }; } } assertGameplaySeason(this.state, event.aggregateType); const result = action(); appended = this.ledger.append({ ...event, payload: result }); const persistedStateUpdatedAt = await this.persistState(client); await client.query("COMMIT"); this.persistedStateUpdatedAt = persistedStateUpdatedAt; this.ledger.markPersisted(); this.commands.commit(); return { alreadyApplied: false, result }; }
    catch (error) { await client.query("ROLLBACK").catch(() => undefined); this.state = previousState; this.logistics.restore(previousLogistics); this.espionage.restore(previousEspionage); this.onboarding.restore(previousOnboarding); this.commands.rollback(); if (appended) this.ledger.discard(appended.id); throw error; } finally { client.release(); }
  }
  ...
  async moderatePlayer(playerId: string, status: "active" | "banned", reason: string, audit?: AdminAuditContext): Promise<{ status: "active" | "banned"; alreadyApplied: boolean }> { return this.runExclusive(() => this.moderatePlayerUnlocked(playerId, status, reason, audit)); }
  private async moderatePlayerUnlocked(playerId: string, status: "active" | "banned", reason: string, audit?: AdminAuditContext): Promise<{ status: "active" | "banned"; alreadyApplied: boolean }> {
    ...
    if (!this.pool) { if (!player) throw new Error("PLAYER_NOT_FOUND"); if ((player.status ?? "active") === status) return { status, alreadyApplied: true }; this.setPlayerStatus(playerId, status, frozenAt); this.ledger.append({ eventType: status === "banned" ? "player.ban" : "player.unban", aggregateType: "player", aggregateId: playerId, payload: { reason, status } }); return { status, alreadyApplied: false }; }
    const client = await this.pool.connect();
    try {
      await client.query("BEGIN"); await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`state:${this.state.kingdom.id}`]); await this.load({ skipLedger: true, strict: true, client }); player = this.findPlayer(playerId); previousState = structuredClone(this.state); previousLogistics = this.logistics.capture(); previousEspionage = this.espionage.capture(); const locked = await client.query("SELECT p.status,u.banned_at FROM players p LEFT JOIN users u ON u.id=p.user_id WHERE p.id=$1 FOR UPDATE OF p", [playerId]);
      if (!player || !locked.rows[0]) throw new Error("PLAYER_NOT_FOUND");
      if (locked.rows[0]?.status === status) {
        await client.query("INSERT INTO admin_actions(id,actor_id,action_type,reason,target_type,target_id,outcome,auth_method,request_id,metadata) VALUES($1,$2,$3,$4,'player',$5,'already_applied',$6,$7,'{}'::jsonb)", [randomUUID(), audit?.actorId ?? null, status === "banned" ? "player.ban" : "player.unban", reason, playerId, audit?.authMethod ?? "legacy_token", audit?.requestId ?? null]);
        await client.query("COMMIT"); return { status, alreadyApplied: true };
      }
      ...
      await client.query("UPDATE auth_sessions SET revoked_at=COALESCE(revoked_at,now()),updated_at=now() WHERE player_id=$1 AND revoked_at IS NULL", [playerId]);
      await client.query("INSERT INTO admin_actions(id,actor_id,action_type,reason,target_type,target_id,outcome,auth_method,request_id,metadata) VALUES($1,$2,$3,$4,'player',$5,'applied',$6,$7,'{}'::jsonb)", [randomUUID(), audit?.actorId ?? null, status === "banned" ? "player.ban" : "player.unban", reason, playerId, audit?.authMethod ?? "legacy_token", audit?.requestId ?? null]);
      event = this.ledger.append({ eventType: status === "banned" ? "player.ban" : "player.unban", aggregateType: "player", aggregateId: playerId, payload: { reason, status } }); const persistedStateUpdatedAt = await this.persistState(client); await client.query("COMMIT"); this.persistedStateUpdatedAt = persistedStateUpdatedAt; this.ledger.markPersisted(); return { status, alreadyApplied: false };
    } catch (error) { await client.query("ROLLBACK").catch(() => undefined); this.state = previousState; this.logistics.restore(previousLogistics); this.espionage.restore(previousEspionage); if (event) this.ledger.discard(event.id); throw error; } finally { client.release(); }
  }
  ...
  async finalizeIfDue(options: { force?: boolean; reason?: string; expectedSeasonId?: string; actorId?: string; authMethod?: "admin_session" | "legacy_token"; requestId?: string } = {}): Promise<boolean> { const now = Date.now(); if (!this.pool && options.expectedSeasonId && options.expectedSeasonId !== this.state.season.id) throw new Error("STALE_SEASON"); if (!this.pool && (this.state.season.status !== "ACTIVE" || (!options.force && Date.parse(this.state.season.endsAt) > now))) return false; const previousState = structuredClone(this.state); const previousLogistics = this.logistics.capture(); const previousEspionage = this.espionage.capture(); if (!this.pool) { const result = this.prepareFinalization(now); hardReset(this.state, newSeason()); this.logistics.resetForSeason(this.state); this.espionage.resetForSeason(); this.commands.clear(); this.ledger.append({ eventType: "season.finalized", aggregateType: "season", aggregateId: result.seasonId, payload: { checksum: result.checksum, resetTemplate: seasonResetTemplate } }); return true; } return this.finalizeInDatabase(now, options, previousState, previousLogistics, previousEspionage); }
  private async finalizeInDatabase(now: number, options: { force?: boolean; reason?: string; expectedSeasonId?: string; actorId?: string; authMethod?: "admin_session" | "legacy_token"; requestId?: string }, previousState: GameState, previousLogistics: ReturnType<LogisticsRepository["capture"]>, previousEspionage: ReturnType<EspionageRepository["capture"]>): Promise<boolean> { const client = await this.pool!.connect(); try { await client.query("BEGIN"); await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`state:${this.state.kingdom.id}`]); const current = await client.query<{ state: GameState }>("SELECT state FROM game_state WHERE state_key='kingdom'"); const currentSeasonId = current.rows[0]?.state.season.id; if (!currentSeasonId) throw new Error("DATABASE_STATE_MISSING"); if (options.expectedSeasonId && options.expectedSeasonId !== currentSeasonId) throw new Error("STALE_SEASON"); await this.load({ skipLedger: true, strict: true, client }); if (this.state.season.id !== currentSeasonId) throw new Error("DATABASE_STATE_UNAVAILABLE"); if (this.state.season.status !== "ACTIVE" || (!options.force && Date.parse(this.state.season.endsAt) > now)) { await client.query("ROLLBACK"); return false; } await client.query("SELECT pg_advisory_xact_lock(hashtext($1))", [`${this.state.kingdom.id}:${currentSeasonId}`]); await client.query("INSERT INTO seasons (id, kingdom_id, status, starts_at, ends_at, config) VALUES ($1,$2,'ACTIVE',$3,$4,$5) ON CONFLICT (id) DO NOTHING", [currentSeasonId, this.state.kingdom.id, this.state.season.startsAt, this.state.season.endsAt, JSON.stringify({ resetTemplate: seasonResetTemplate })]); const claimed = await client.query("UPDATE seasons SET status='FINALIZING' WHERE id=$1 AND finalized_at IS NULL AND status IN ('ACTIVE','FINALIZING') RETURNING id", [currentSeasonId]); if (!claimed.rowCount) { await client.query("ROLLBACK"); return false; } const result = this.prepareFinalization(now); await this.persistSeasonResult(client, result); hardReset(this.state, newSeason()); this.logistics.resetForSeason(this.state); this.espionage.resetForSeason(); await this.persistSeasonReset(client, result.seasonId, result.closedAt, options); this.ledger.append({ eventType: "season.finalized", aggregateType: "season", aggregateId: result.seasonId, payload: { checksum: result.checksum, resetTemplate: seasonResetTemplate } }); const persistedStateUpdatedAt = await this.persistState(client); await client.query("COMMIT"); this.persistedStateUpdatedAt = persistedStateUpdatedAt; this.ledger.markPersisted(); this.commands.clear(); return true; } catch (error) { await client.query("ROLLBACK").catch(() => undefined); this.state = previousState; this.logistics.restore(previousLogistics); this.espionage.restore(previousEspionage); throw error; } finally { client.release(); } }
  ...
  private async persistSeasonReset(client: PoolClient, closedSeasonId: string, closedAt: string, options: { force?: boolean; reason?: string; actorId?: string; authMethod?: "admin_session" | "legacy_token"; requestId?: string }): Promise<void> { for (const player of this.state.players) await client.query("INSERT INTO player_reputation (player_id, score) VALUES ($1,$2) ON CONFLICT (player_id) DO UPDATE SET score=EXCLUDED.score", [player.id, player.crossSeasonReputation]); const playerIds = this.state.players.map(player => player.id); if (playerIds.length) { // Caravans, cargo and trade routes survive the season boundary with the
    // in-memory state (resetForSeason keeps them), so they are not deleted here.
    await client.query("DELETE FROM spy_cooldowns WHERE player_id = ANY($1::uuid[])", [playerIds]); await client.query("DELETE FROM counter_intel_active WHERE player_id = ANY($1::uuid[])", [playerIds]); } await client.query("DELETE FROM diplomacy_treaties WHERE kingdom_id=$1", [this.state.kingdom.id]); await client.query("DELETE FROM espionage_actions WHERE kingdom_id=$1", [this.state.kingdom.id]); await client.query("DELETE FROM world_events WHERE kingdom_id=$1", [this.state.kingdom.id]); await client.query("UPDATE seasons SET status='CLOSED', finalized_at=$2 WHERE id=$1", [closedSeasonId, closedAt]); await client.query("INSERT INTO seasons (id, kingdom_id, status, starts_at, ends_at, config) VALUES ($1,$2,'ACTIVE',$3,$4,$5)", [this.state.season.id, this.state.kingdom.id, this.state.season.startsAt, this.state.season.endsAt, JSON.stringify({ resetTemplate: seasonResetTemplate })]); if (options.force) await client.query("INSERT INTO admin_actions(id,actor_id,action_type,reason,target_type,target_id,outcome,auth_method,request_id,metadata) VALUES($1,$2,'season.close',$3,'season',$4,'applied',$5,$6,'{}'::jsonb)", [randomUUID(), options.actorId ?? null, options.reason ?? "manual close", closedSeasonId, options.authMethod ?? "legacy_token", options.requestId ?? null]); }
