...
export class GameStore {
  ...
  async load(options?: { skipLedger?: boolean; strict?: boolean; client?: PoolClient }): Promise<void> {
    ...
    if (executor) {
      ...
      } catch (error) { if (options?.strict) throw error; console.warn("database load skipped", error instanceof Error ? error.message : error); }
    }
    ...
    this.worldEvents.seed(this.state); this.raiders.seed(this.state);
