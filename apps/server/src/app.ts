...
export function createServer(): { app: FastifyInstance; store: GameStore; start: () => Promise<void>; stop: () => Promise<void> } {
  ...
  const playerFromRequest = async (request: { headers: { authorization?: string } }) => { const token = request.headers.authorization?.replace(/^Bearer\s+/i, ""); if (!token) return undefined; if (config.authMode === "dev") return devTokens.get(token); const principal = await auth.authenticateAccess(token, "player"); return principal?.kind === "player" ? principal.playerId : undefined; };
  ...
  app.addHook("onRequest", async (request, reply) => { if (config.authMode !== "password" || request.method !== "POST") return; const url = request.raw.url ?? ""; if (!url.startsWith("/api/auth/") && !url.startsWith("/api/admin/auth/")) return; const origin = request.headers.origin; if (!origin || origin !== config.clientOrigin) return reply.code(403).send({ code: "ORIGIN_NOT_ALLOWED" }); });
  ...
  app.post<{ Body: { username?: string; password?: string } }>("/api/auth/login", async (request, reply) => { if (config.authMode !== "password" || !store.databasePool) return reply.code(404).send({ code: "NOT_FOUND" }); const username = normalizeUsername(request.body?.username ?? ""); const limited = await rateLimited(`login:${request.ip}:${username}`, 5, 900_000); if (limited) return reply.code(limited === "RATE_LIMITED" ? 429 : 503).send({ code: limited }); const row = await auth.findUser(username); const validPassword = await verifyPassword(request.body?.password ?? "", row?.password_hash ?? await dummyPasswordHash()); if (!row || row.role !== "player" || !row.player_id || row.player_status !== "active" || !validPassword || row.status === "banned") return reply.code(401).send({ code: "INVALID_CREDENTIALS" }); const user = { kind: "player" as const, id: row.id, username, playerId: row.player_id, status: row.status }; let session; try { session = await auth.createSession(user, row.password_hash); } catch (error) { if (error instanceof Error && error.message === "STALE_CREDENTIALS") return reply.code(401).send({ code: "INVALID_CREDENTIALS" }); throw error; } reply.setCookie("refresh_token", session.refreshToken, { httpOnly: true, sameSite: "strict", secure: config.cookieSecure, path: "/api/auth", maxAge: Math.floor(REFRESH_MS / 1000) }); return { token: session.accessToken, accessExpiresAt: session.accessExpiresAt, player: store.findPlayer(user.playerId), snapshot: getSnapshot(user.playerId) }; });
  app.post("/api/auth/refresh", async (request, reply) => { if ((request.headers.origin ?? "") !== config.clientOrigin) return reply.code(403).send({ code: "ORIGIN_NOT_ALLOWED" }); if (config.authMode !== "password") return reply.code(404).send({ code: "NOT_FOUND" }); const token = (request.headers.cookie ?? "").match(/(?:^|; )refresh_token=([^;]+)/)?.[1]; if (!token) return reply.code(401).send({ code: "UNAUTHORIZED" }); const limited = await rateLimited(`refresh:${request.ip}`, 30, 60_000); if (limited) return reply.code(limited === "RATE_LIMITED" ? 429 : 503).send({ code: limited }); const session = await auth.rotateRefresh(token, "player"); if (!session || session.user.kind !== "player") { reply.clearCookie("refresh_token", { path: "/api/auth" }); return reply.code(401).send({ code: "UNAUTHORIZED" }); } reply.setCookie("refresh_token", session.refreshToken, { httpOnly: true, sameSite: "strict", secure: config.cookieSecure, path: "/api/auth", maxAge: Math.floor(REFRESH_MS / 1000) }); return { token: session.accessToken, accessExpiresAt: session.accessExpiresAt, player: store.findPlayer(session.user.playerId), snapshot: getSnapshot(session.user.playerId) }; });
  app.post("/api/auth/logout", async (request, reply) => { if ((request.headers.origin ?? "") !== config.clientOrigin) return reply.code(403).send({ code: "ORIGIN_NOT_ALLOWED" }); const token = (request.headers.cookie ?? "").match(/(?:^|; )refresh_token=([^;]+)/)?.[1]; if (token) await auth.revokeRefresh(token, "player"); reply.clearCookie("refresh_token", { path: "/api/auth" }); return { ok: true }; });
  ...
  const moderateAdminPlayer = async (request: any, reply: any, status: "active" | "banned", context: { actorId?: string; authMethod: "admin_session" | "legacy_token"; requestId: string }, playerId: string, reason: string) => { try { const result = await store.moderatePlayer(playerId, status, reason, context); requestBroadcast(); if (status === "banned") { const message: ServerMessage = { type: "SNAPSHOT", payload: getSnapshot(playerId) }; const payload = JSON.stringify(message); for (const [token, tokenPlayerId] of devTokens) if (tokenPlayerId === playerId) devTokens.delete(token); for (const [socket, connectedPlayerId] of clients) if (connectedPlayerId === playerId) socket.send(payload, error => { if (error) socket.terminate(); else socket.close(4401, "ACCOUNT_BANNED"); }); } return { accepted: true, playerId, status: store.findPlayer(playerId)?.status, frozen: status === "banned", alreadyApplied: result.alreadyApplied }; } catch (error) { if (error instanceof Error && error.message === "PLAYER_NOT_FOUND") return reply.code(404).send({ code: "PLAYER_NOT_FOUND" }); request.log.error(error); return reply.code(500).send({ code: "MODERATION_FAILED" }); } };
  const closeAdminSeason = async (request: any, reply: any, context: { actorId?: string; authMethod: "admin_session" | "legacy_token"; requestId: string }, seasonId: string, reason: string) => { try { const finalized = await store.runExclusive(() => store.finalizeIfDue({ force: true, reason, expectedSeasonId: seasonId, ...context })); requestBroadcast(); return { accepted: finalized, status: finalized ? "finalized" : "already_finalized", season: { id: store.snapshot.season.id, status: store.snapshot.season.status } }; } catch (error: any) { const message = error instanceof Error ? error.message : "INVALID_REQUEST"; if (message === "STALE_SEASON") return reply.code(409).send({ code: message }); request.log.error(error); return reply.code(500).send({ code: "INTERNAL_ERROR" }); } };
  registerAdminRoutes({ app, auth, config, store: () => store, rateLimited, moderate: moderateAdminPlayer, closeSeason: closeAdminSeason });
