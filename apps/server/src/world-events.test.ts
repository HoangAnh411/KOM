...
import { plagueCatchUpLimit, plaguePeriodMs } from "./world-events.js";
...
test("plague is periodic, composition-aware, and bounded after offline catch-up", () => {
  const store = new GameStore();
  const army = store.snapshot.armies[0]!;
  const start = 1_000_000;
  const event = store.worldEvents.spawn(store.snapshot, "plague", 13, start);
  event.severity = 1;
  event.affectedTiles = [{ x: army.x, y: army.y }];
  event.endsAt = new Date(start + plaguePeriodMs * 100).toISOString();
  delete event.lastPlagueAt;
  store.worldEvents.seed(store.snapshot);
  assert.equal(event.lastPlagueAt, event.startsAt, "legacy event receives a persisted baseline");
  store.worldEvents.tick(store.snapshot, start + plaguePeriodMs - 1);
  assert.equal(army.strength, 100, "sub-period ticks do no damage");
  store.worldEvents.tick(store.snapshot, start + plaguePeriodMs * 2);
  assert.equal(army.strength, 90);
  assert.equal(army.composition?.frontline?.count, 90);
  store.worldEvents.tick(store.snapshot, start + plaguePeriodMs * 20);
  assert.equal(army.strength, 90 - 5 * plagueCatchUpLimit);
  const caughtUp = army.strength;
  store.worldEvents.tick(store.snapshot, start + plaguePeriodMs * 20);
  assert.equal(army.strength, caughtUp, "immediate retries cannot drain capped backlog in chunks");
