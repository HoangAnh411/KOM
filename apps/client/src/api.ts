...
export type SocketHandlers = {
  ...
  /** Fires on every state transition: open→connecting, first snapshot→online, loss→reconnecting, browser offline→offline. */
...
export function openSocket(handlers: SocketHandlers): { close: () => void } {
  ...
  const emit = (event: ConnectionEvent) => { state = reduceConnection(state, event); handlers.onConnectionState(state); };
  const connect = () => {
    if (stopped) return;
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (message.type === "SNAPSHOT" && awaitingAuth) { awaitingAuth = false; emit({ type: "authed" }); }
      switch (message.type) {
  ...
  // Browser-level network loss surfaces as "offline"; coming back reconnects now.
  const onOffline = () => emit({ type: "offline" });
  const onOnline = () => {
    emit({ type: "online" });
    attempt = 0;
    if (socket) { try { socket.close(1000, "network restored"); } catch { /* already closed */ } socket = undefined; }
    connect();
  };
  if (typeof window !== "undefined") { window.addEventListener("offline", onOffline); window.addEventListener("online", onOnline); }
  connect();
  return {
    close: () => {
      stopped = true;
      if (typeof window !== "undefined") { window.removeEventListener("offline", onOffline); window.removeEventListener("online", onOnline); }
 ...
 * so retries after network failure are idempotent). Parses the CommandResponse
 * on BOTH success and error bodies — an early-rejected (401/403/429) response
 * still carries { commandId, result: "rejected", code }. */
export async function sendCommand(token: string, path: string, body: Record<string, unknown>, commandId: string, signal?: AbortSignal): Promise<CommandResponse> {
  const response = await authorizedFetch(token, apiBase + path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ commandId, ...body }), signal });
  const payload = (await response.json().catch(() => ({}))) as Partial<CommandResponse> & { code?: string };
  if (!response.ok && !payload.commandId) throw new ApiError(payload.code ?? "INVALID_REQUEST");
  const settled = { commandId, ...payload };
  if (settled.snapshot) snapshotSink?.(settled.snapshot);
