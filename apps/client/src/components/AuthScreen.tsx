...
function DevLogin() {
  ...
  return <form className="login-card" onSubmit={event => {
    ...
    void api.login(name, faction)
      ...
      .catch(reason => addNotice(reason instanceof Error ? reason.message : "Đăng nhập thất bại"));
  }}><h1>Kingdoms of Meridian</h1><p>Chiến thắng bằng chiến lược, kinh tế và ngoại giao.</p><input value={name} onChange={event => setName(event.target.value)} placeholder="Tên người chơi" /><select value={faction} onChange={event => setFaction(event.target.value as FactionId)}>{Object.entries(factions).map(([id, item]) => <option value={id} key={id}>{item.name}</option>)}</select><button type="submit">Vào kingdom</button></form>;
}
