  ...
  return <Panel accent="teal" className="logistics-panel" panelRef={anchor} aria-label={"Kinh tế & vận tải"}>
    ...
    <PanelBody>
      ...
      <div className="logistics-block export-note">
        ...
        <p className="kom-meta">Lập tuyến đến <strong>{marketHubs.find(hub => hub.id === destId)?.name ?? "một thương cảng"}</strong> và gửi caravan. Khi hàng cập bến, lượng gỗ/đá/sắt được tính là xuất khẩu — góp vào điểm kinh tế và mục tiêu “Đã xuất khẩu” trong phần giới thiệu. Băng cướp có thể phục kích trên đường: cử quân hộ tống đem theo caravan để bảo vệ.</p>
      </div>
    </PanelBody>
  </Panel>;
}
