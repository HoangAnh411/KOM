...
- [ ] typecheck sạch; server unit xanh (**132 sau P0.2**, sẽ tăng tiếp ở P0.3a); client 78 + shared 3 xanh; 19 e2e xanh trên port 3100/5174; `check:bundle` ≤ 500 KiB/chunk
...
- [ ] **P0.3a** (= S-3, phần 1) Một registry dedupe có trần, thay ba Set `claim()` của repo — ⚠️ hot file · M · deps P0.2 (dùng chung window — đã có `config.idempotencyWindow`) · **Bước 9**. Đọc code 2026-09-03 thì S-3 rộng hơn bản ghi cũ: **năm** cơ chế dedupe song song, và **ba** trong số đó bị sao chép **hai lần mỗi command** (`store.ts:96` capture cho rollback + `store.ts:99` `load()` dựng lại) — `CombatRepository.commands` (`combat.ts:104`, 6 call site), `LogisticsRepository.commands` (`logistics.ts:103`, 5), `OnboardingRepository.commands` (`onboarding.ts:92`, 1), cộng `state.processedCommands` bị `structuredClone` 2×. Chỉ `EventLedger.commandIds` có trần (P0.2). Sửa: `command-registry.ts` mới (Set trần FIFO: `has` / `claim` / `forget(ids)` / `clear`), ba repo nhận registry qua constructor và `claim()` chỉ còn delegate — **không đổi call site nào**; rollback đổi từ copy cả Set sang `forget()` danh sách id đã claim trong transaction đó
